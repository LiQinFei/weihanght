<template>
  <div class="commonOrder" v-loading.fullscreen.lock="fullscreenLoading">
    ...
    <el-table ref="multipleTable" :data="tableData.dataList" border style="width: 100%" v-loading="loading">
      ...
      <el-table-column prop="deliveryStatus" :formatter="deliveryStatus" label="物流状态" width="100">
      </el-table-column>
      <el-table-column prop="orderPayAmount" label="支付总额" width="100">
      </el-table-column>
      <el-table-column label="配送方式" width="140" show-overflow-tooltip>
        <template scope="scope">
          <span v-if="scope.row.deliveryAddress.needDelivery == 'true'">配送</span>
          <span v-else-if="scope.row.deliveryAddress.needDelivery == 'false'">自提</span>
        </template>
      </el-table-column>
      ...
      <el-table-column fixed="right" label="操作" width="150">
        <template scope="scope">
          <el-button @click="sendGoods(scope.row)" v-if="(scope.row.deliveryStatus == 1) && (scope.row.deliveryAddress.needDelivery != 'false')" type="text" size="small">
            发货
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog title="订单详情" :visible.sync="showVisible">
      ...
      <el-form label-position="left" inline class="demo-table-expand">
        ...
        <el-form-item label="配送方式">
          <span v-if="showUer.needDelivery">配送</span>
          <span v-else>自提</span>
        </el-form-item>
